...
- [x] **Kartu testimoni visual** — foto serah-terima menjadi media utama dengan fallback foto unit/ilustrasi lokal, rating dan unit tampil sebagai overlay, identitas serta cerita tersusun modern, dan satu testimoni otomatis terpusat. Klik kartu tetap membuka modal detail.
- [x] **Carousel testimoni responsif** — homepage mengambil maksimal 5 testimoni terbaru; 4 kartu tampil di laptop lebar, 3 di laptop/tablet landscape, 2 di tablet, dan 1 dengan peek di HP. Mendukung tombol prev/next, swipe, scroll snap, dan centering saat item lebih sedikit dari kapasitas layar.
- [x] **Foto testimoni tanpa crop** — preview create/edit menghormati rasio dan `object-contain`; modal cerita memakai gambar penuh responsif dengan batas tinggi viewport. Kartu tetap memakai crop 4:3 yang lebih tinggi sebagai thumbnail visual.
- [x] **Flag transaksi histori Unit & Order** — badge konsisten `POST LEDGER` / `REFERENCE ONLY` tampil pada list dan detail. Tabel Unit meminta record reference-only secara eksplisit, sedangkan mode kartu mengecualikannya pada API dan guard render lokal tanpa perubahan schema.
- [x] **Validasi alasan backdate Order** — form pembayaran serta perubahan status DEAL/Batal menampilkan jumlah karakter yang masih kurang sampai minimum 5 karakter terpenuhi.
- [x] **QA terbaru** — suite frontend lulus 75 test, lint tanpa error (9 warning lama), dan production build berhasil.
