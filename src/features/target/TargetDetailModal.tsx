...
export const TargetDetailModal = ({ open, onClose, targetId, branchHeader, branchKey }: Props) => {
  const { can } = usePermissions();
  const { data: targetRes, isLoading } = useTargetBranch(targetId ?? undefined, branchHeader, branchKey);
  const target = targetRes?.data as BranchTarget | undefined;

  const isClosed = target?.status === 'CLOSED';
  const { data: achRes } = useTargetAchievement(
    !isClosed ? target?.period : undefined,
    branchHeader,
    branchKey,
  );
  ...
  if (achData && target) {
    const single = 'breakdown' in achData ? achData.breakdown.find((b) => b.branchId === target.branchId) : achData;
  ...
  const scopedBranchHeader = target ? { 'X-Branch-Id': target.branchId } : undefined;
  const { data: lookupRes } = useTargetLookupSales(scopedBranchHeader, open && !!target && target.status === 'DRAFT');
  ...
  useEffect(() => {
    if (open && target) {
      setRows(rowsFromSalesTargets(target.salesTargets));
      setDistError(null);
  ...
  }, [open, target?.id, target?.salesTargets]);

  if (!open) return null;

  const sumUnit = rows.reduce((t, r) => t + (r.unitTarget || 0), 0);
  const sumRevenue = rows.reduce((t, r) => t + (r.revenueTarget || 0), 0);
  const diffUnit = (target?.unitTarget ?? 0) - sumUnit;
  const diffRevenue = (target?.revenueTarget ?? 0) - sumRevenue;
  ...
  const saveDistribution = () => {
    ...
    m.replaceSales.mutate({ id: target.id, data: { sales } }, { onError: handleDistError });
  };
  ...
  return (
    <>
      <Modal
        ...
        subtitle={target ? `${target.branch?.nama ?? target.branchId} · ${target.period}` : undefined}
        size="xl"
        footer={
