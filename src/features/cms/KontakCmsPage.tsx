...
export const KontakCmsPage = () => {
  ...
  return (
    <RequirePermission code="SITE_SETTING_READ">
      <div className="max-w-[1200px] mx-auto  space-y-5">
        ...
        <SectionCard title="Informasi Utama" icon={<Globe size={16} />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <TextField label="Alamat Lengkap" value={form.address} onChange={(e) => set('address', e.target.value)} placeholder="Jl. Raya..." disabled={!can('SITE_SETTING_UPDATE')} />
            </div>
            <TextField label="Nomor Telepon" value={form.phone} onChange={(e) => set('phone', e.target.value)} placeholder="021-xxx" disabled={!can('SITE_SETTING_UPDATE')} />
            <TextField label="Email" value={form.email} onChange={(e) => set('email', e.target.value)} placeholder="email@domain.com" disabled={!can('SITE_SETTING_UPDATE')} />
            <TextField label="Jam Operasional" value={form.businessHours} onChange={(e) => set('businessHours', e.target.value)} placeholder="Senin–Sabtu, 09.00–18.00" disabled={!can('SITE_SETTING_UPDATE')} />
          </div>
        </SectionCard>
        ...
        <SectionCard title="WhatsApp" icon={<MessageCircle size={16} />}>
          <div className="space-y-4">
            <TextField label="Nomor WhatsApp (tanpa +)" value={form.whatsappNumber} onChange={(e) => set('whatsappNumber', e.target.value)} placeholder="628xxx" disabled={!can('SITE_SETTING_UPDATE')} />
            {form.whatsappNumber && (
        ...
        <SectionCard title="Media Sosial" icon={<Globe size={16} />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <TextField label="Instagram (URL)" value={form.socialInstagram} onChange={(e) => set('socialInstagram', e.target.value)} placeholder="https://instagram.com/..." disabled={!can('SITE_SETTING_UPDATE')} />
            <TextField label="Facebook (URL)" value={form.socialFacebook} onChange={(e) => set('socialFacebook', e.target.value)} placeholder="https://facebook.com/..." disabled={!can('SITE_SETTING_UPDATE')} />
          </div>
        </SectionCard>
        ...
        <SectionCard title="Peta Lokasi" icon={<MapPin size={16} />}>
          <div className="space-y-4">
            <TextField label="URL Embed Peta (OpenStreetMap / Google Maps)" value={form.mapEmbedUrl} onChange={(e) => set('mapEmbedUrl', e.target.value)} placeholder="https://..." disabled={!can('SITE_SETTING_UPDATE')} />
            {form.mapEmbedUrl && (
