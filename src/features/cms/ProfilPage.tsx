...
import { useAbout, useAboutMutations } from './cms.hooks';
import type { About, CmsStat, CmsIconItem } from './cms.types';
...
export const ProfilPage = () => {
  ...
  useEffect(() => { if (data && !about) setAbout(structuredClone(data)); }, [data, about]);

  if (isLoading || !about) {
    if (isError) return <div className="text-center py-24 text-muted font-semibold text-sm">Gagal memuat halaman Tentang.</div>;
    return <div className="flex items-center justify-center py-24 text-muted"><Loader2 size={24} className="animate-spin" /></div>;
  }

  const setVisiMisi = (key: 'visi' | 'misi', value: string) => setAbout((p) => (p ? { ...p, [key]: value } : p));

  const handleSave = () => about && update.mutate(about, { onError: (err) => notifyApiError(err) });

  const openAddStat = () => { setFIconKey('car'); setFValue(''); setFLabel(''); setForm({ mode: 'stat', index: null }); };
  const openEditStat = (i: number) => { const s = about.stats[i]; setFIconKey(s.icon ?? 'car'); setFValue(s.value); setFLabel(s.label); setForm({ mode: 'stat', index: i }); };
  const openAddValue = () => { setFIconKey('shield-check'); setFTitle(''); setFDesc(''); setForm({ mode: 'value', index: null }); };
  const openEditValue = (i: number) => { const v = about.values[i]; setFIconKey(v.icon); setFTitle(v.title); setFDesc(v.desc); setForm({ mode: 'value', index: i }); };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setAbout((p) => {
      ...
      if (form.mode === 'stat') {
        ...
        const stats = form.index === null ? [...p.stats, entry] : p.stats.map((s, i) => (i === form.index ? entry : s));
        return { ...p, stats };
      }
      const entry: CmsIconItem = { icon: fIconKey, title: fTitle, desc: fDesc };
      const values = form.index === null ? [...p.values, entry] : p.values.map((v, i) => (i === form.index ? entry : v));
      return { ...p, values };
    });
    setForm(null);
  };
  ...
  const handleDelete = () => {
    ...
    setAbout((p) => {
      if (!p) return p;
      if (deleteTarget.mode === 'stat') return { ...p, stats: p.stats.filter((_, i) => i !== deleteTarget.index) };
      return { ...p, values: p.values.filter((_, i) => i !== deleteTarget.index) };
    });
    ...
    <div className="max-w-[1200px] mx-auto animate-float-up space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <SectionCard title="Visi" icon={<Eye size={16} />}>
          <textarea value={about.visi} onChange={(e) => setVisiMisi('visi', e.target.value)} rows={4}
        ...
        <SectionCard title="Misi" icon={<Target size={16} />}>
          <textarea value={about.misi} onChange={(e) => setVisiMisi('misi', e.target.value)} rows={4}
                ...
                return (
                  <button key={opt.key} type="button" onClick={() => setFIconKey(opt.key)}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[11px] font-bold border transition-colors ${
                      fIconKey === opt.key ? 'bg-primary text-white border-primary' : 'bg-surface-soft border-border text-ink-soft hover:border-primary'
                    }`}>
