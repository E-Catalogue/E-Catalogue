...
const ExpenseReportPageInner = () => {
  ...
  const cards = [
    ...
    ['Payroll', s?.payrollExpense ?? 0], ['Rekondisi Tambahan', s?.additionalReconditioningExpense ?? 0],
  ...
  const columns: Column<ExpenseDetailRow>[] = [
    { header: 'Tanggal', cell: (row) => formatDate(row.date) }, { header: 'Tipe', cell: (row) => row.type },
  ...
  return <div className="max-w-[1600px] mx-auto space-y-5 pb-12">
    ...
    {(report.data?.legacy.generatedRecurringWithoutSource || report.data?.legacy.completedReconditioningWithoutTimestamp) ? <div className="rounded-xl border border-accent-amber/30 bg-accent-amber/10 px-4 py-3 text-[12px] font-semibold text-accent-amber">Data legacy di luar metrik bertanggal: rutin tanpa sumber {report.data.legacy.generatedRecurringWithoutSource}, rekondisi tanpa tanggal selesai {report.data.legacy.completedReconditioningWithoutTimestamp}.</div> : null}
