      ...
      {run && (
        <div className="space-y-4">
          ...
          <DataTable columns={[
            ...
            { header: 'Tunjangan', align: 'right' as const, cell: (i: PayrollItem) => run.status === 'DRAFT' ? <input key={i.id} type="text" inputMode="numeric" defaultValue={i.allowance ? i.allowance.toLocaleString('id-ID') : ''} placeholder="0" onInput={(e) => { const digits = e.currentTarget.value.replace(/\D/g, ''); e.currentTarget.value = digits ? Number(digits).toLocaleString('id-ID') : ''; }} onBlur={(e) => updateItem(i, 'allowance', e.target.value)} className="w-28 h-9 px-2 rounded-lg bg-surface border border-border text-right" /> : formatCurrency(i.allowance) },
            { header: 'Potongan', align: 'right' as const, cell: (i: PayrollItem) => run.status === 'DRAFT' ? <input key={i.id} type="text" inputMode="numeric" defaultValue={i.deduction ? i.deduction.toLocaleString('id-ID') : ''} placeholder="0" onInput={(e) => { const digits = e.currentTarget.value.replace(/\D/g, ''); e.currentTarget.value = digits ? Number(digits).toLocaleString('id-ID') : ''; }} onBlur={(e) => updateItem(i, 'deduction', e.target.value)} className="w-28 h-9 px-2 rounded-lg bg-surface border border-border text-right" /> : formatCurrency(i.deduction) },
