...
export const Modal = ({ open, onClose, title, subtitle, icon, children, footer, size = 'md' }: ModalProps) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div
        className={`relative z-10 w-full ${SIZES[size]} bg-surface rounded-t-3xl sm:rounded-3xl shadow-card-hover flex flex-col max-h-[92vh] sm:max-h-[88vh] animate-modal-in`}
        role="dialog"
        aria-modal="true"
      >
        {(title || icon) && (
