...
```json
{
  ...
  "image": { "filename": "unit1.jpg" }, "createdAt": "..."
}
```
...
| Beranda | `GET /public/homepage` *(hero, brands, why-us, how-it-works, featured+units, testimonials+items, cta — semua sekaligus)* |
...
| — *(belum ada halaman)* | `/cms/homepage/why-us`, `/how-it-works`, `/brands`, `/featured` | **belum ada UI** |
...
| — *(belum ada)* | `/cms/about/hero`, `/cms/about/cta` | **belum ada UI** |
| **Info Kontak** (`KontakCmsPage`) | `/cms/site-settings` (+ `/cms/contact-page`) | wired |
| — *(belum ada)* | Inbox `/cms/contact-messages` | **belum ada UI** |
| **Katalog** (`KatalogPage` CMS) | `/cms/catalog*` (+ `/cms/catalog-page`) | wired |
| — *(belum ada)* | `/cms/credit-simulation/config` | **belum ada UI** |
